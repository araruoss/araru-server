...
export async function obterObra(id, { userId = null } = {}) {
  ...
  const { rows: files } = await query(`SELECT wf.*, f.payload, f.source, f.library_id AS "libraryId", f.library_source_id AS "librarySourceId", f.content_hash AS "contentHash", f.hash_algorithm AS "hashAlgorithm", f.pipeline_status AS "pipelineStatus", f.pipeline_stage AS "pipelineStage", f.pipeline_error_code AS "pipelineErrorCode", f.pipeline_error AS "pipelineError", f.manifest, f.manifest_version AS "manifestVersion" FROM work_files wf JOIN library_files f ON f.id=wf.file_id WHERE wf.work_id=$1 AND ${fileScope.sql} ORDER BY wf.is_primary DESC, wf.format`, [id, ...fileScope.values]);
  return { ...work, authors: parse(work.authors, []), tags: parse(work.tags, []), files: files.map((row) => ({ ...parse(row.payload), id: row.file_id, source: row.source, libraryId: row.libraryId, librarySourceId: row.librarySourceId, contentHash: row.contentHash, hashAlgorithm: row.hashAlgorithm, pipelineStatus: row.pipelineStatus, pipelineStage: row.pipelineStage, pipelineErrorCode: row.pipelineErrorCode, pipelineError: row.pipelineError, manifest: parse(row.manifest, {}), manifestVersion: row.manifestVersion, formato: row.format || parse(row.payload).formato || '', workId: row.work_id, isPrimary: row.is_primary })) };
}
