...
import { hashPassword, validatePassword } from './userAuthService.js';

const allowedLanguages = new Set(['en', 'pt-BR']);
...
export async function getSetupStatus() {
  ...
  return { setupRequired: !completed, completed, supportedLanguages: [...allowedLanguages], supportedThemes: [...allowedThemes], defaultTheme: 'dark', publicSettings: settings ? { language: settings.language, theme: settings.theme, libraryName: settings.libraryName } : null };
}

export async function completeSetup(input = {}) {
  const language = allowedLanguages.has(input.language) ? input.language : 'en';
  const theme = allowedThemes.has(input.theme) ? input.theme : 'dark';
  ...
  return withTransaction(async (client) => {
    ...
    await client.query('INSERT INTO users(id,username,email,display_name,password_hash,password_changed_at,role,role_id,profile_id,must_change_password,active) VALUES($1,$2,$3,$4,$5,NOW(),$6,$7,$8,FALSE,TRUE)', [userId, username, email, input.admin?.displayName || username, hashPassword(password), 'admin', 'role-administrator', profileId]);
...
export async function getGeneralSettings() { const { rows } = await query("SELECT value FROM system_settings WHERE key='general'"); return rows[0]?.value || { language: 'en', theme: 'dark', libraryName: 'Araru' }; }
export async function saveGeneralSettings(settings = {}, userId) {
  ...
  const value = {
    libraryName,
    language: allowedLanguages.has(settings.language) ? settings.language : (current.language || 'en'),
    theme: allowedThemes.has(settings.theme) ? settings.theme : (current.theme || 'dark'),
    timezone,
